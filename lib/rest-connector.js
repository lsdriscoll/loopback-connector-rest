...
exports.initialize = function initializeDataSource(dataSource, callback) {
    ...
    settings.baseURL = settings.baseURL || settings.restPath || 'http://localhost:3000/';


    var connector = new RestConnector(settings);
 ...
 * @param {string} config.baseURL The base URL
 * @param {boolean} config.debug The debug flag
 * @param {string} config.responseDataProperty The property of the response body that contains the requested data
 * @constructor
 */
function RestConnector(config) {
    this._baseURL = config.baseURL;
    this._models = {};
    this._resources = {};
    this._debug = config.debug;
	this._responseDataProperty = config.responseDataProperty || 'data';
}
...
RestConnector.prototype.responseHandler = function(model, callback) {
	...
	return function (err, response) {
		// if error here then the connector has failed
		if (err) {
			callback && callback(err);
			return;
		}

		// if error here then request has failed
		if (response.error) {
			callback && callback(response.error);
			return;
		}

		// if the response has no body then the rest server has a bug
		if (!response.body){
			callback && callback('No body in response. Indicates a bug in the remote server');
			return;
		}

		// if the response body contains an error something is wrong but it might not be a bug
		if(response.body.error){
			callback && callback(response.body.error);
			return;
		}

		// the response should have the responseDataProperty
		if(!response.body.hasOwnProperty(self.responseDataProperty)){
			callback && callback('No data in response. Please check configured `responseDataProperty`');
			return;
		}

		var data = response.body[self.responseDataProperty];

		self.postProcess(model, data);
		callback(null, data);
	};
};
